...
def simulate_ml(tile_size=50, fps=2, max_steps=50):
    ...
    pygame.display.set_caption("ML-based Simulation")
    ...
    while running and step < max_steps and world.agent_alive and not world.gold_found:
        ...
        # Update Q-table (reward: -1 per move; -100 for death; +100 for gold)
        reward = -1
        if not world.agent_alive:
            reward = -100
        elif world.gold_found:
            reward = 100
        agent.update_q_table(current_pos, action, reward, new_pos)
        step += 1
